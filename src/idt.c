...
void idt_init(void) {
    // GDT 설정 (Ring 0, Ring 3 코드/데이터, TSS 포함)
    gdt_set_entry(0, 0, 0, 0, 0); // Null
    gdt_set_entry(1, 0, 0, 0x9A, 0x20); // Ring 0 Code
    gdt_set_entry(2, 0, 0, 0x92, 0x00); // Ring 0 Data
    gdt_set_entry(3, 0, 0, 0xFA, 0x20); // Ring 3 Code
    gdt_set_entry(4, 0, 0, 0xF2, 0x00); // Ring 3 Data
    
    // TSS 설정 (인덱스 5, 셀렉터 0x28)
    uint64_t tss_base = (uint64_t)&tss;
    gdt_set_entry(5, sizeof(tss), tss_base, 0x89, 0x00);
    
    // TSS 스택 설정 (Ring 0 스택 주소)
